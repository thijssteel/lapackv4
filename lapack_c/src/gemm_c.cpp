...
using namespace lapack_cpp;
    ...
    #define INSTANTIATE_C_GEMM(T, name_lower, name_upper)                    \
        ...
        {                                                                    \
            ...
            ConstMatrix<T, lapack_idx> A_(transa_ == Op::NoTrans ? m : k,    \
                                          transa_ == Op::NoTrans ? k : m, A, \
                                          lda);                              \
            ConstMatrix<T, lapack_idx> B_(transb_ == Op::NoTrans ? k : n,    \
                                          transb_ == Op::NoTrans ? n : k, B, \
                                          ldb);                              \
            Matrix<T, lapack_idx> C_(m, n, C, ldc);                          \
